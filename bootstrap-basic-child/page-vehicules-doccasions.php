...
<form action="" method="post">
	...
	<select class="select-filtre col-md-12" name="kilometrage">
	<option value="">Kilométrage</option>
		<?php for  ($i=50000; $i < 350001; $i+=50000) {
			echo '<option value="'.$i.'" ';
			if (isset($_POST['kilometrage']) && $_POST['kilometrage']==$i) {
				echo ' selected ';
			}
			echo '> < '.$i.' km</option>';
		}
		...
		<option value="350001"> > 350000 km</option>
	</select>
	...
	if ($_POST['kilometrage']!='' && isset($_POST['kilometrage'])) {
		$filtreKm = $_POST['kilometrage'];
		$compareKm = '<';

		if ($_POST['kilometrage']==350001) {
			$compareKm = '>';
		}
	}
	else {
	...
	if( $my_query->have_posts()) :
		...
		?> 
		<div class="background-image-liste-vehicule"><i class="fa fa-eye fa-3x"></i><img class="image-liste-vehicule" src="<?php echo $image[0]->guid;  ?>" alt="<?php echo $image[0]->post_title;  ?>" /></div>
		...
		<strong class="legende-liste-vehicule">Kilométrage :</strong> <strong class="champ-liste-vehicule"> <?php the_field('km');?> km</strong><br>
		</div>
		</div>
		</a>
		</div>
